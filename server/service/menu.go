...
func (menuService *MenuService) AssignRoleMenus(menus []dbTable.Menu, RoleId uint) error {
	r := dbTable.Role{ID: RoleId}
	// global.DB.Preload("Menus").First(&r, "id = ?", RoleId)
	err := global.DB.Model(&r).Association("Menus").Replace(menus)
	return err
}

func (menuService *MenuService) GetRoleMenus(roleId uint) (menus []dbTable.Menu, err error) {
	//TODO: filter out unused field by define GetRoleMenus response struct
	err = global.DB.Model(&dbTable.Role{ID: roleId}).Association("Menus").Find(&menus)
	return menus, err
}
